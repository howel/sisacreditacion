 ...
 function indexBuscarAlumno($query,$p,$c,$fac) 
    { //echo $fCac;
        ...
        $semestre=$ct['semestre_actual'];
         ...
         $sql = "SELECT alumnos.CodigoAlumno, alumnos.NombreAlumno,
 concat(alumnos.ApellidoPaterno,' ',alumnos.ApellidoMaterno) AS Apellidos, 
concat(alumnos.TipoDocumento,' ',alumnos.NumDocumento) AS Documento, alumnos.FechaIngreso, alumnos.CodAlumnoSira, 
case when detalle_asignacion_tutoria.CodigoProfesor is null then 'Falta' else 'Asignado' end Estado,
case when estado_chek_asignacion_tutoria.estado_chek is not null  then '1'else '0' end Estado_chek
FROM
alumnos
Inner Join matricula ON alumnos.CodigoAlumno = matricula.CodigoAlumno
Left Join detalle_asignacion_tutoria ON detalle_asignacion_tutoria.CodigoAlumno = alumnos.CodigoAlumno
left Join estado_chek_asignacion_tutoria ON estado_chek_asignacion_tutoria.CodigoAlumno = alumnos.CodigoAlumno
