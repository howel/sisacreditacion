...
class asistenciaalumnoEU extends Main {
 ...
 Inner Join clasificacion_evento ON clasificacion_evento.id_clasificacion_evento = tipo_evento.id_clasificacion_evento
    where  evento.CodigoSemestre='" . $semestre_ultimo . "' and  clasificacion_evento.id_clasificacion_evento=3  or  tipo_evento.idtipo_evento= 6 and " . $c . " like :query";
...
WHERE evento.idevento='" . $idevento . "' and (evento.idtipo_evento=4 or evento.idtipo_evento=6 )"
        ;
...
//        echo "<pre>";print_r($_P);exit;
        $sql2 = $this->Query("act_asistencia_externoEU(:p1,:p2,:p3)");
...
//                echo "id ex ".$_P['id_externo'][$key];echo "-";  echo "check".$_P['chek'][$_P['id_externo'][$key]];echo "<br>";
      ...
      function update_docente($_Pd) {
        $sql2 = $this->Query("act_asitencia_docenteEU(:p1,:p2,:p3)");
        $stmt2 = $this->db->prepare($sql2);
        try {
            $this->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $this->db->beginTransaction();
...
//                echo "id ex ".$_P['id_externo'][$key];echo "-";  echo "check".$_P['chek'][$_P['id_externo'][$key]];echo "<br>";
                $stmt2->bindValue(':p1', $_Pd['CodigoProfesor'][$key], PDO::PARAM_INT);
...
$sql2 = $this->Query("act_asistencia_alumEU(:p1,:p2,:p3)");
        $stmt2 = $this->db->prepare($sql2);
        try {
            $this->db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $this->db->beginTransaction();
