...
<div class="contain">
                    ...
                    if ($edit) {///si le pusiste true a esitar
                        if ($presidente) {
                            ...
                            if ($value['estado_modo_vista'] == false) { // si no recive true a la consta sql en estado_vista => es un usuario normal
                                ?>
                            <td align="center" width="5%" ><a href="index.php?controller=<?php echo $name; ?>&action=edit&id=<?php echo $value[0]; ?>" title="Editar"><img alt="" src="images/add.png" /></a></td>
                            <?php
                        } else {
    ...
    if ($cheket_estado_asignacion) {
        if ($value['Estado_chek'] == '1') {
            $cheked = "checked";
        }
        if ($value['Estado_chek'] == '0') {
            $cheked = "";
        }
